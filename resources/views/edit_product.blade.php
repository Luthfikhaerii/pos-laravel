...
<head>
    ...
    <title>Edit Product - CafféPoint POS</title>
...
<body class="bg-gray-50">
    <div class="flex min-h-screen">
        ...
        <main class="flex-1 flex flex-col min-h-screen ml-48">
            <!-- Header Section -->
            <header class="bg-white border-b z-20 border-gray-200 sticky top-0 z-10 shadow-sm pt-4">
            ...
            <div class="flex-1 bg-gray-50 p-6 overflow-y-auto">
                <form class="bg-white rounded-lg shadow-md p-8" method="POST" action="{{ route('product.update',['id'=>request()->route('id')]) }}" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="flex items-start gap-4">
                            ...
                            <div class="relative w-32 h-32">
                                ...
                                <label for="file-upload" 
                                    class="absolute inset-0 flex flex-col items-center justify-center border-2 border-dashed border-gray-300 rounded-lg cursor-pointer hover:border-blue-500 hover:bg-blue-50 transition-all duration-200 z-0">
                                    <svg class="w-10 h-10 text-gray-400" fill="none" stroke="currentColor" stroke-width="2" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4" />
                                    </svg>
                                    <span class="text-xs text-gray-500 mt-2 font-medium">Upload</span>
                                </label>
                                
                                <!-- Current Image (if exists) -->
                                @if(isset($data->image) && $data->image)
                                <div id="current-image" class="absolute inset-0 z-10 pointer-events-none">
                                    <img src="{{ asset('/storage/' . $data->image) }}" alt="Current product" class="w-full h-full object-cover rounded-lg border-2 border-gray-300">
                                    <div class="absolute top-1 right-1 px-2 py-0.5 bg-gray-800 text-white text-xs rounded">Current</div>
                                </div>
                                @endif
                                
                                <!-- New Image Preview -->
                                <div id="image-preview" class="hidden absolute inset-0 z-20">
                                    <img id="preview-img" src="" alt="Preview" class="w-full h-full object-cover rounded-lg border-2 border-blue-500">
                                    <button type="button" id="remove-image" 
                                            class="absolute -top-2 -right-2 w-6 h-6 bg-red-500 text-white rounded-full flex items-center justify-center hover:bg-red-600 transition-colors shadow-lg z-30">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" />
                                        </svg>
                                    </button>
                                    <div class="absolute top-1 right-1 px-2 py-0.5 bg-blue-600 text-white text-xs rounded">New</div>
                                </div>
                            </div>
                            ...
                            <div class="flex-1">
                                ...
                                @if(isset($data->image) && $data->image)
                                <p class="text-xs text-blue-600 mt-2">💡 Click the box to upload a new image</p>
                                @endif
                            </div>
                        </div>
