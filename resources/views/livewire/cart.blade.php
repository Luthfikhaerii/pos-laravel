<div class="bg-white shadow-sm w-80 h-screen fixed right-0 rounded-sm flex flex-col justify-between">
    {{ var_dump($cart) }}
    <p>{{$count}}</p>
    <div class="p-4 h-20 flex items-center">
    ...
    <div class="flex-1 overflow-y-auto p-4 space-y-3">
        ...
        <div class="flex items-center justify-between border-b border-gray-200 px-2 pb-3">
            <div class="">
                <p class="font-semibold text-gray-800">{{$item['name_product']}}</p>
                <p class="text-sm text-gray-500 truncate">{{$item['category']}}</p>
                <p class="text-blue-500 font-semibold">Rp{{$item['price']}}</p>
            </div>
            ...
            <div class="flex items-center space-x-2">
                ...
                <span class="w-6 text-center font-medium">{{$item['amount']}}</span>
